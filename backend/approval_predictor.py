...
CARD_THRESHOLDS: dict[str, tuple[int, float, float]] = {
    "hdfc_infinia":      (780, 2_500_000, 0.45),
    ...
    "amex_platinum":     (780,  2_000_000, 0.40),
...
def _generate_training_data(n_per_card: int = 500) -> pd.DataFrame:
    ...
    for card_id, (min_cibil, min_income, base_rate) in CARD_THRESHOLDS.items():
        for _ in range(n_per_card):
            ...
            income = float(rng.integers(100_000, 50_000_000)) # Up to 5 Cr
            n_cards = int(rng.integers(0, 7))

            # Approval probability modulated by distance from thresholds
            cibil_gap  = (cibil  - min_cibil)  / 150   # normalized
            income_gap = (income - min_income) / max(min_income, 500_000)
            
            # Base probability from thresholds
            # We want prob to be ~0.7 at the threshold gap=0
            prob = base_rate + 0.3 + 0.3 * np.tanh(cibil_gap) + 0.1 * np.tanh(income_gap)
            
            # HEAVY penalties for being below absolute minimums
            if cibil < min_cibil:
                prob -= 0.6 * ((min_cibil - cibil) / 120)**2
            if income < min_income:
                # Quadratic penalty for missing income
                prob -= 0.7 * ((min_income - income) / max(min_income, 1_000_000))**2

            # Too many cards increases risk
            prob -= 0.05 * max(0, n_cards - 4)
...
def _approval_reason(
    ...
    elif cibil < min_cibil + 10:
        parts.append(f"CIBIL {cibil} is just at the threshold — maintain good payment history to secure approval.")
...
def predict_approval(
    ...
    for card_id in target_ids:
        ...
        if clf is not None:
            ...
            # Handle single-class RF
            if len(clf.classes_) == 1:
                prob = float(clf.classes_[0])
            else:
            ...
            # HARD OVERRIDE: If you don't meet the min CIBIL or Income, 
            # the probability shouldn't be high, regardless of what the RF says.
            min_c, min_i, _ = CARD_THRESHOLDS.get(card_id, (700, 300_000, 0.5))
            if cibil_score < min_c - 10: 
                prob = min(prob, 0.08) # Cap at 8%
            if income_annual < min_i * 0.7: 
                prob = min(prob, 0.05) # Cap at 5%
            
            # Debug log
            if "infinia" in card_id:
                print(f"[DEBUG] Infinia Predict: cibil={cibil_score}, income={income_annual}, prob={prob}")

            # Extreme case
            if cibil_score < 650 or (income_annual < 300000 and min_i > 500000):
                prob = min(prob, 0.01) # Near zero
