...
function Leaderboard() {
  ...
  useEffect(() => {
    aiApi.get("/leaderboard/cities").then(({ data }) => {
...
function SubmitStack() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await aiApi.post("/submit-combo", {
        cards:     cardList,
        city,
        persona,
...
function OfferVotes() {
  ...
  const fetchOffers = async (cardId = cardFilter) => {
    ...
    try {
      ...
      const { data } = await aiApi.get(`/offers${params}`);
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const { data } = await aiApi.post("/offers", {
